...
public class SimpleGlobalItemScorer extends AbstractGlobalItemScorer {
    ...
    public void globalScore(@Nonnull Collection<Long> items, @Nonnull MutableSparseVector scores) {
        ...
        for(VectorEntry e: scores.fast(VectorEntry.State.EITHER)){
        	//e.getKey() to fetch item id since scores is item id to score mapping
        	List<ScoredId> nbrs = model.getNeighbors(e.getKey());
        	double summation = 0.0;
        	for(ScoredId nbr : nbrs){
        		if(items.contains(nbr.getId())){
        			summation += nbr.getScore();
        		}
        	}
        	scores.set(e.getKey(), summation);
