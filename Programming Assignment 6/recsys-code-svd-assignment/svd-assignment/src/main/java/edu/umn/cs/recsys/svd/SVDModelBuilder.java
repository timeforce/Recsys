...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        RealMatrix U_trunc = U.getSubMatrix(0, U.getRowDimension() - 1, 0, featureCount - 1);
        RealMatrix V_trunc = V.getSubMatrix(0, V.getRowDimension() - 1, 0, featureCount - 1);
        RealMatrix S_trunc = S.getSubMatrix(0, featureCount - 1, 0, featureCount - 1);
        ...
        return new SVDModel(userMapping, itemMapping, U_trunc, V_trunc, S_trunc);
    }
    ...
    private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
        ...
        try {
            for (UserHistory<Event> user: users) {
                 ...
                 * Score items in a vector. The key domain of the provided vector is the items to score, 
                 * and the score method sets the values for each item to its score (or unsets it, if no score can be provided). 
                 * The previous values are discarded.
                 * 
                 */
                baselineScorer.score(user.getUserId(), baselines);
         
                // TODO Populate this user's row with their ratings, minus the baseline scores
                for(long item: ratings.keySet()){// get rated items
