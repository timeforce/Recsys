    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
            ...
            if(model.getUserVector(user) == null){
            	scores.clear(); //if a user has not rated an item, leave that entry blank
            } else {
            	RealMatrix score = 
            			model.getUserVector(user).multiply(model.getFeatureWeights()).
            			multiply(model.getItemVector(item).transpose()).scalarAdd(baselineScorer.score(user, item));
            	//score is an one-element matrix
