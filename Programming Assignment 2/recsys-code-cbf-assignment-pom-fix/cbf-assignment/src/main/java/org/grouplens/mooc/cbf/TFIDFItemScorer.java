    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null && p.getValue() >= 3.5) {
            	...
            	SparseVector itemVector = model.getItemVector(itemId);
            	profile.add(itemVector);
            }
        }
        
        /**
         * 1. Compute meanRating as μu=∑rui / userRatings.size().
         * 2. For each rating rui:
         * 	1. Compute weight as rui−μu.
         *  2. Create a mutable copy of the item vector. The name of the result vector is weightedItemVector.
         *  3. Multiply weightedItemVector by weight.
         *  4. Add weightedItemVector to profile.
         */
        
        //rewrite above part to implement weighted user profile
        // Iterate over the user's ratings to build their profile
        /*
        ...
        for (Rating r: userRatings) {
            // In LensKit, ratings are expressions of preference
            Preference p = r.getPreference();
            total_rating += p.getValue();
            
        }
        double mean_rating = total_rating / userRatings.size();
        
        for (Rating r: userRatings){
        	Preference p = r.getPreference();
        	double rui = p.getValue();
        	double weight = rui - mean_rating;
        	long itemId = p.getItemId();
        	SparseVector itemVector = model.getItemVector(itemId);
        	//create a mutable copy of item vector so that we can multiply it with weight later
        	MutableSparseVector weightedItemVector = itemVector.mutableCopy();
